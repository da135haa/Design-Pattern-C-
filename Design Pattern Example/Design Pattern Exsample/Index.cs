...
這個原則是給不允許多重繼承的程式語言使用的，因為物件只允許單一繼承，所以最多只擁有一個父物件的功能，但這不利於軟體的重覆使用性，所以就有合成/聚合重覆使用原則出現
聚合則是一個 has-a 關係,整體與部分的關係,表示"含有"整體由部分組合而成,部分可以脫離主體而單獨存在
合成是一個 is-a 關係,是一種更強的聚合,部分組成主體而且不可分割,生命週期也一樣
重點就是要想清楚繼承的關係該怎麼建立,善用介面的多重繼承
https://read01.com/R5M78G.html
